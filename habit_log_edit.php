...
<div class="container main-content">
    <div class="page-header animate-fade-in">
        <nav aria-label="breadcrumb" style="background: none; padding: 0; margin: 0 0 15px 0; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
            <a href="dashboard.php" style="color: var(--text-secondary); text-decoration: none; display: flex; align-items: center; gap: 5px;"><i class="fas fa-home"></i> Dashboard</a>
            <i class="fas fa-chevron-right" style="font-size: 0.8rem; color: var(--text-secondary);"></i>
            <a href="habit.php" style="color: var(--text-secondary); text-decoration: none; display: flex; align-items: center; gap: 5px;">Habits</a>
            <i class="fas fa-chevron-right" style="font-size: 0.8rem; color: var(--text-secondary);"></i>
            <a href="habit_view_logs.php?habit_id=<?= $log['habit_id'] ?>" style="color: var(--text-secondary); text-decoration: none; display: flex; align-items: center; gap: 5px;"><?= htmlspecialchars($log['habit_name']) ?></a>
            <i class="fas fa-chevron-right" style="font-size: 0.8rem; color: var(--text-secondary);"></i>
    ...
    <?php elseif (isset($_GET['error']) && $_GET['error'] === 'duplicate'): ?>
        <div class="alert alert-warning animate-fade-in"> A log for the selected date already exists. </div>
    <?php endif; ?>

    <div class="form-container animate-fade-in">
        <form action="habit_log_edit.php?log_id=<?= $log_id ?>" method="post">
            <div class="mb-4">
                <label for="log_date" class="form-label"><i class="fas fa-calendar-alt text-secondary me-2"></i>Date</label>
                <input type="date" id="log_date" name="log_date" class="form-control" required value="<?= htmlspecialchars($log['log_date']) ?>" />
            </div>
            <div class="mb-4">
                <label for="status" class="form-label"><i class="fas fa-check-circle text-secondary me-2"></i>Status</label>
            ...
            <div class="mb-4">
                <label for="note" class="form-label"><i class="fas fa-comment text-secondary me-2"></i>Note (optional)</label>
                <input type="text" id="note" name="note" class="form-control" placeholder="Add a note..." value="<?= htmlspecialchars($log['note']) ?>" />
            </div>
            <div class="form-actions">
                <button type="submit" class="btn btn-journal"><i class="fas fa-save me-2"></i> Update Log</button>
                <a href="habit_view_logs.php?habit_id=<?= $log['habit_id'] ?>" class="btn btn-outline-journal"><i class="fas fa-times me-2"></i> Cancel</a>
            </div>
        </form>
    </div>
</div>
